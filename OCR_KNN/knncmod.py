...
def train():
	...
	bigset=[np.ones((2224,193))*-1,np.ones((2224,193))*-1,np.ones((2223,193))*-1]
	count=1
	l1=[]
	l2=[]
	l3=[]
	...
	for k in range(1,11):
		...
		for i in range(0,r):
			...
			for entry in valid:
				...
				if k==1:
					...
					for m in range(0,k):
						...
						di=l[m][0]
						try:
							a[l[m][1]]+=float((dk-di))/(dk-d1)
						except ZeroDivisionError:
							print dk,d1,di,":",k-1,0,m
				maximum=0
				label=-1
				for each in a:
					if maximum <= a[each]:
			...
			error.append(err)
		mean=reduce(lambda x, y: x + y, error) / len(error)
	...
	for line in f:
		...
		for i in a:
			if maximum < a[i]:
				...
				label=i
		if label!=b[192]:
			print "Wrong prediction: Predicted = ",label, " Actual = ",b[192]
		else:
